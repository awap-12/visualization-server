...
function incrementArrayItems(array, diff) {
    array.forEach((item, i, a) => a[i] = item + diff);
}
...
module.exports = {
    arrayDiff(arrayA, arrayB, comparisonFn = (x, y) => utils.isDeepStrictEqual(x,y)) {
        if (!Array.isArray(arrayA) || !Array.isArray(arrayB))
            throw new TypeError("`arrayA` and `arrayB` required");
